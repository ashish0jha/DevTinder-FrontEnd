      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="flex bg-[#11131c] border border-zinc-800/60 p-1 rounded-xl mb-8">
          <button
            ...
            className={`flex-1 py-2 text-[13px] font-medium rounded-lg transition-all duration-150 cursor-pointer ${activeTab === 'received'
          ...
          <button
            ...
            className={`flex-1 py-2 text-[13px] font-medium rounded-lg transition-all duration-150 cursor-pointer ${activeTab === 'sent'
                ? "bg-zinc-800 text-zinc-100 shadow-sm"
                : "text-zinc-400 hover:text-zinc-200"
              }`}
          >
          ...
          {activeTab === 'received' && (
            receivedRequests?.length === 0 ? (
              ...
              receivedRequests?.map((req) => {
                ...
                return (<ReceivingRejectingRequest key={req._id} id={req._id} request={req.senderId} receivedRequests={receivedRequests} setReceivedRequests={setReceivedRequests}/>)
              })
            )
          )}
          ...
          {activeTab === 'sent' && (
            sentRequests.length === 0 ? (
              ...
              sentRequests.map((req) => {
                return (<SentRequestSection req={req.receiverId} requestId={req._id} key={req._id} />)
              })
            )
          )}
        </div>
